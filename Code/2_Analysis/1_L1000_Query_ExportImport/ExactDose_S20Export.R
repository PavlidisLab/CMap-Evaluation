# L1000-Query Bulk Export Script (Exact Dose Comparison; Lower Bound - S20)
...
inDir = 'Data/Subset/'
queryDir = 'L1000_S20/Query/'
workDir = 'L1000_S20/WorkList/'
logDir = 'L1000_S20/WorkList/LOGS/'
...
for (currentBatch in 1:finalBatch) {
	...
	for (currentIndex in startIndex:endIndex) {
		...
		currentName = cmapMini[currentIndex, sprintf('S20.%s_%s_%s', cell_id, trt_iname, case_ID)]
		
		geneList = rownames(cmapFC)
		upMask = frankv(x = cmapFC[, currentID], order = -1, na.last = NA, ties.method = 'first') %in% (1:10)
		upGenes = geneList[upMask]
		downMask = frankv(x = cmapFC[, currentID], order = 1, na.last = NA, ties.method = 'first') %in% (1:10)
