# L1000-Query Bulk Export Script (Ignore Dose Comparison; 10-150 Gene Threshold)
...
inDir = 'Data/Subset/'
queryDir = 'L1000_Ignore/Query/'
touchstoneDir = 'L1000_Ignore/Touchstone/'
workDir = 'L1000_Ignore/WorkList/'
logDir = 'L1000_Ignore/WorkList/LOGS/'
...
# Main Subroutine -------------------------------------------------------
cat('Loading Data.\n')
tempPath = sprintf('%sfinal.subset.RDAT.XZ', inDir)
load(tempPath)
...
for (currentBatch in 1:finalBatch) {
	...
	for (currentIndex in startIndex:endIndex) {
		...
		currentName = cmapMini[currentIndex, sprintf('%s_%s_%s', cell_id, trt_iname, case_ID)]
		
		geneList = rownames(cmapFC)
		upGenes = geneList[(cmapFC[, currentID] >= 1)]
		downGenes = geneList[(cmapFC[, currentID] <= -1)]
